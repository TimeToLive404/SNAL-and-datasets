...
class MLP_decision(nn.Module):
    def __init__(self, input_dim=173056):
...
def scene_evalue_cal(output, frame, inp_dim, pattern='train', commom_evalue=0, i=0, Alpha=1, Beta=1):
    # 将bbox由神经网络输出坐标映射回真实图像坐标
    # 求缩放系数
    img_dim = frame.shape[1], frame.shape[0]
    try:
        img_dim = torch.FloatTensor(img_dim).repeat(1, 2).cuda().repeat(output.size(0), 1)
    except:
        if pattern == 'train':
            return commom_evalue
        else:
            return 0
    scaling_factor = torch.min(int(inp_dim) / img_dim, 1)[0].view(-1, 1)
    # 将（416*416）下的坐标转换成（416*320）这种无填充的坐标
    output[:, [0, 2]] = output[:, [0, 2]] - (int(inp_dim) - scaling_factor * img_dim[:, 0].view(-1, 1)) / 2
    output[:, [1, 3]] = output[:, [1, 3]] - (int(inp_dim) - scaling_factor * img_dim[:, 1].view(-1, 1)) / 2
    # 坐标缩放
    output[:, 0:4] = output[:, 0:4] / scaling_factor

    # 将锚框画在图像上，并标注
    area_dif = 0
    if pattern == 'train':  # 需要计算常见场景特征值
        commom_evalue = commom_evalue[0]
        for one_box in output:  # 每个框都计算一次
            one_box = one_box.int().cpu().numpy().tolist()
            c1 = tuple(one_box[0:2])
            c2 = tuple(one_box[2:4])
            ########
            classes = load_classes('D:/SNAL/data/coco.names')
            label = "{0}".format(classes[int(one_box[-1])])  # 当前框的类别
            colors = pkl.load(open("D:/SNAL/pallete", "rb"))
            cv2.rectangle(frame, c1, c2, (0, 255, 0), 1)  # 当前物体识别框
            text_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_PLAIN, 1, 1)[0]
            # c2 = c1[0] + text_size[0] + 3, c1[1] + text_size[1] + 4
            # cv2.rectangle(frame, c1, c2, (0, 255, 0), -1)  # 标签框
            # cv2.putText(frame, label, (c1[0], c1[1] + text_size[1] + 4), cv2.FONT_HERSHEY_PLAIN, 1, [225, 255, 255], 1)
            # cv2.imshow(f'picture', frame)
            # print('按q以外的任意键继续')
            # while True:
            #     key = cv2.waitKey(1)
            #     if key == ord('q'):
            ...
            #         break
            commom_evalue += (c2[0] - c1[0]) * (c2[1] - c1[1]) / (i + 1)
            box_evalue = (c2[0] - c1[0]) * (c2[1] - c1[1]) - commom_evalue
            area_dif += box_evalue
        # print(output)
        try:
            commom_evalue = commom_evalue / len(output)  # 场景平均特征值n_hat
        except:
            commom_evalue = commom_evalue
        scene_evalue = Beta * area_dif + commom_evalue * Alpha * len(output)  # 场景的特征相似度值S_hat
        return commom_evalue, scene_evalue

    elif pattern == 'test':
        for i, one_box in enumerate(output):  # 每个框都计算一次
            one_box = one_box.int().cpu().numpy().tolist()
            c1 = tuple(one_box[0:2])
            c2 = tuple(one_box[2:4])
            classes = load_classes('D:/SNAL/data/coco.names')
            ...
            box_evalue = (c2[0] - c1[0]) * (c2[1] - c1[1]) - commom_evalue
            area_dif += box_evalue
        scene_evalue = Beta * area_dif + commom_evalue * Alpha * len(output)  # 场景的特征相似度值
        return scene_evalue


# 以下为参考YOLOv3该部分的代码,与本实验无关,仅供参考
...
# 场景特征值计算
def scene_eigenvalue(yolo_output, num_class, conf_thre, nms_thre, img, pattern='train', common_evalue=0, i=0):
    output = output_handle(yolo_output, num_class=int(num_class), conf_thre=float(conf_thre),
                           nms_thre=float(nms_thre))
    scene_evalue = scene_evalue_cal(output, img, inp_dim=416, pattern=pattern, commom_evalue=common_evalue, i=i)
    return scene_evalue
...
def Mineuron(Iapp, num_frame):
    global v, n, t, qt, time
    if num_frame % 1 == 0:  # 重新初始化
        t = 0.0  # 设置初始时间
        v = vR  # 设置初始电量
        n = 0.05
        qt = []  # 用来盛放差分得到的q值
    ...
    for i in range(4):
        ...
        time.append(t)  # 时间列表
    qt.sort()
    return qt[-1]  # 输出qt中最大的v


class Snal(nn.Module):
    def __init__(self):
        super(Snal, self).__init__()
