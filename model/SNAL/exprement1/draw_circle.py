...
# SNAL最佳预测
plt.subplot(2, 3, 2)
prediction = get_pre('D:/SNAL/result/log_test1.txt')
color = color_mapping(predictions=prediction)
plt.scatter(x_circle, y_circle, s=100, c=color, cmap='PuBu_r')  # YIBu_r
cb = plt.colorbar()
cb.set_label('Vigilance')  # 设置colorbar的标签
plt.gca().invert_yaxis()
plt.title('SNAL')

# Darknet最佳预测
plt.subplot(2, 3, 3)
prediction = get_pre('D:/MyCNN/result/log_cnn.txt')
color = color_mapping(predictions=prediction)
plt.scatter(x_circle, y_circle, s=100, c=color, cmap='PuBu_r')  # YIBu_r
cb = plt.colorbar()
cb.set_label('Vigilance')  # 设置colorbar的标签
...
# RNN
plt.subplot(2, 3, 4)
prediction = get_pre('D:/MyLSTM/result/log_rnn.txt')
color = color_mapping(predictions=prediction)
plt.scatter(x_circle, y_circle, s=100, c=color, cmap='PuBu_r')  # YIBu_r
cb = plt.colorbar()
cb.set_label('Vigilance')  # 设置colorbar的标签
...
# LSTM
plt.subplot(2, 3, 5)
prediction = get_pre('D:/MyRNN/result/log_lstm.txt')
color = color_mapping(predictions=prediction)
plt.scatter(x_circle, y_circle, s=100, c=color, cmap='PuBu_r')  # YIBu_r
cb = plt.colorbar()
cb.set_label('Vigilance')  # 设置colorbar的标签
...
# ViT
plt.subplot(2, 3, 6)
prediction = get_pre('D:/MyViT/result/log_vit.txt')
color = color_mapping(predictions=prediction)
plt.scatter(x_circle, y_circle, s=100, c=color, cmap='PuBu_r')  # YIBu_r
cb = plt.colorbar()
cb.set_label('Vigilance')  # 设置colorbar的标签
