...
def arg_init():
    ...
    parser.add_argument('--truck_car_dataset', dest='tc_datafile', help='数据集地址', default='D:/data/random_train_type/',
                        type=str)  # random_train_type
    parser.add_argument('--learning_rate', dest='lr', help='学习率', default='0.001')
    parser.add_argument('--save_path', dest='savefile', help='模型参数保存地址', default='D:/save_for_mylstm', type=str)
    parser.add_argument('--inp_dim', dest='inp_dim', help='网络要处理的图片尺寸', default='416', type=str)
    parser.add_argument('--epoch', dest='epochs', help='模型训练轮数', default='7', type=str)
...
def train(cnn, lstm, traindata, loss_function, optimizer, epochs, h_prev, visual=None, random=True):
    lstm.train()
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adam(lstm.parameters(), lr=float(args.lr))
    epochs = int(args.epochs)
    visual = args.visual
    h_prev = torch.zeros(1, 3, 20)  # (layer?,batch_size,hidden_size)

    train(cnn, lstm, traindata, loss_function, optimizer, epochs, h_prev, visual)
    save_model()
